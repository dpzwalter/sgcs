...
function BrowserLayout(cfg) {
	...
	this.DIVStart = function (element, divId, divClasses, divStyle, draggable, selectable, captureKeyboard) {
		...
		if (cfg.isEditor() && element != null) {
			...
			onClickFunction = !selectable ? '' : ' onclick="' + cfg.getMe() + '.SelectElementClicking(event, \'' + element.ElementId + '\');" onmouseover="' + cfg.getMe() + '.MouseOver(event, \'' + element.ElementId + '\');" onmouseout="' + cfg.getMe() + '.MouseOut(event, \'' + element.ElementId + '\');"';
