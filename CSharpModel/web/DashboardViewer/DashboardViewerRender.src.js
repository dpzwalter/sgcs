...
var ControlType = { Editbox: "Edit", Checkbox: "Check box", RadioButton: "Radio button", ComboBox: "Combo box", Slider: "Slider", Range: "Range", DropDownList: "Drop-down list", Switch: "Switch" };
var DataType = { Integer: "Numeric (without decimals)", Real: "Numeric (with decimals)", Character: "Character", Boolean: "Boolean", Date: "Date", DateTime: "DateTime", GUID: "GUID" };
...
var QueryViewerAxisType = { Rows: "Row", Columns: "Column", Pages: "Page", Data: "Data", Hidden: "NotShow" };
...
function DashboardViewer($) {
	...
	this.BasicWidgetDefaults = function (widget, dashboard, widgetType) {
		...
		widget.FrameTitle = widget.FrameTitle || "";
	...
	this.ObjectElementDefaults = function (element, dashboard) {
		...
		element.Visible = element.Visible != undefined ? element.Visible : true;		// se trata diferente por ser booleana con default = true
	...
	this.FilterDefaults = function (filter, dashboard) {
		...
		filter.Caption = filter.Caption || filter.Name;
		...
		filter.Control.EmptyItemText = filter.Control.EmptyItemText || EditorDefaults.EmptyItemText;
		filter.Control.Orientation = filter.Control.Orientation || ControlOrientation.Horizontal;
		filter.Control.InviteMessage = filter.Control.InviteMessage || "";	//TODO: hay que ocultar el InviteMessage en el caso de Date y DateTime
	...
	this.ImageDefaults = function (image, dashboard) {
		...
		image.AlternateText = image.AlternateText || "";
	...
	this.TextBlockDefaults = function (textBlock, dashboard) {
		...
		textBlock.Caption = textBlock.Caption || "";
	...
	this.ReplaceVariablesInObjectElement = function (element, variables) {
		if (element.Type != QueryViewerAxisType.Hidden)
			element.Title = this.ReplaceVariablesValuesInText(element.Title, variables);
		if ((element.Type == QueryViewerAxisType.Rows || element.Type == QueryViewerAxisType.Columns) && (element.DataType == DataType.Date || element.DataType == DataType.DateTime)) {
		...
		if (element.Type == QueryViewerAxisType.Data) {
		...
		if (element.Type == QueryViewerAxisType.Rows || element.Type == QueryViewerAxisType.Columns) {
		...
		if (!filter.Control.Dynamic && (filter.Control.Type == ControlType.Checkbox || filter.Control.Type == ControlType.ComboBox || filter.Control.Type == ControlType.RadioButton || filter.Control.Type == ControlType.DropDownList)) {
			if (filter.DataType == DataType.Boolean) {
				...
				for (var j = 0; j < filter.Control.Values.length; j++) {
					...
					filter.Control.Descriptions[j] = Parse(this, filter.Control.Descriptions[j], DataType.Character, false, false);
				}
		}
	}
	...
	this.RefreshWidget = function (widget) {
		...
		if (widget.WidgetType == WidgetType.Filter && widget.Control.Type == ControlType.DropDownList)
			$('#' + this.editor.GetDOMElementId(widget.ElementId) + ' > .selectpicker').selectpicker('refresh');
	}
	...
	this.ResolveVariable = function(value, dataType, isCollection) {
		if (value != null) {
			...
			return checkResult.IsVariable ? this.GetEmptyValue(dataType, false) : value;
		}
		else
			return value;
	}
	...
	this.GetImageWidgetHTML = function (image, variables) {
		...
		var src = gx.ajax.imageGuidToUrl(image.Guid);
		if (this.IsDashboardEdit())
			src = "Resources/TempImages/" + src;
	...
	this.GetFilterWidgetHTML = function (filter, variables) {
		...
		function RenderNewRadioButtonFilter(uc, filter, variables) {
			...
			if (NeedToRefreshValues(filter)) {
				...
				if (filter.Control.EmptyItem) {
					...
					html += ValueHTML(uc, uc.ValueToString(value, filter.DataType, filter.Picture, filter.PictureProperties), gx.getMessage(filter.Control.EmptyItemText), uc.SameValue(value, filter.Value, filter.DataType), centerAlign, filter, valueCls);
				}
		...
		function RenderComboBoxFilter(uc, filter, variables) {
			...
			if (needToRefreshValues) {
				...
				if (filter.Control.EmptyItem) {
					var value = uc.GetEmptyValue(filter.DataType, false);
					html += ValueHTML(uc, uc.ValueToString(value, filter.DataType, filter.Picture, filter.PictureProperties), gx.getMessage(filter.Control.EmptyItemText), uc.SameValue(value, filter.Value, filter.DataType), filter);
				}
				for (var i = 0; i < filter.Control.Values.length; i++) {
					var value = filter.Control.Values[i];
					var description = filter.Control.Descriptions[i];
		...
		function RenderDropDownListFilter(uc, filter, variables) {
			...
			if (!needToRefreshValues) {
				if (filter.Control.EmptyItem) {
					var value = uc.GetEmptyValue(filter.DataType, false);
					html += ValueHTML(uc, uc.ValueToString(value, filter.DataType, filter.Picture, filter.PictureProperties), gx.getMessage(filter.Control.EmptyItemText), uc.ValuesIndexOf(filter.Values, value, filter.DataType) >= 0, filter, valueCls);
				}
		...
		function RenderOldRadioButtonFilter(uc, filter, variables) {
			...
			if (NeedToRefreshValues(filter)) {
				...
				if (filter.Control.EmptyItem) {
					var value = uc.GetEmptyValue(filter.DataType, false);
					html += ValueHTML(uc, uc.ValueToString(value, filter.DataType, filter.Picture, filter.PictureProperties), gx.getMessage(filter.Control.EmptyItemText), uc.SameValue(value, filter.Value, filter.DataType), filter, valueCls);
				}
				for (var i = 0; i < filter.Control.Values.length; i++) {
					var value = filter.Control.Values[i];
					var description = filter.Control.Descriptions[i];
	...
	this.ControlValuesLoaded = function (filter, resText) {
		...
		for (var j = 0; j < qViewer.Data.Rows.length; j++) {
			...
			var parserDescriptionResult = this.ParseInternalValue(description, DataType.Character, false);
			if (parserValueResult.Success && parserDescriptionResult.Success) {
				filter.Control.Values.push(parserValueResult.ParsedValue);
				filter.Control.Descriptions.push(parserDescriptionResult.ParsedValue);
			}
		}
	...
	this.CreateSlider = function (filterId) {
		...
		if (filter != null) {
			...
			var filterMaxValue = this.ResolveVariable(filter.MaxValue, filter.DataType, filter.Type == FilterType.Collection);
			var filterMinValue = this.ResolveVariable(filter.MinValue, filter.DataType, filter.Type == FilterType.Collection);
			if (filter.Type == FilterType.Value)
				filterValue = this.ResolveVariable(filter.Value, filter.DataType, filter.Type == FilterType.Collection);
			else {
				filterLowerValue = this.ResolveVariable(filter.LowerValue, filter.DataType, filter.Type == FilterType.Collection);
				filterUpperValue = this.ResolveVariable(filter.UpperValue, filter.DataType, filter.Type == FilterType.Collection);
			}
	...
	this.GetQueryViewerParameters = function (dashboard) {
		...
		function GetParameter(uc, name, value, dataType, isCollection) {
			var valueAux = dataType == DataType.Real ? value.replace(',', '.') : value;
			return { Name: name, Value: uc.ResolveVariable(valueAux, dataType, isCollection) };
		}
		...
		for (var i = 0; i < dashboard.Filters.length; i++) {
			...
			var parameter;
			if (filter.Type != FilterType.Range) {
				parameter = GetParameter(this, filter.Name, filter.SerializedValue, filter.DataType, filter.Type == FilterType.Collection);
				parameters.push(parameter);
			}
			else {
				parameter = GetParameter(this, filter.NameLowerValue, filter.SerializedLowerValue, filter.DataType, filter.Type == FilterType.Collection);
				parameters.push(parameter);
				parameter = GetParameter(this, filter.NameUpperValue, filter.SerializedUpperValue, filter.DataType, filter.Type == FilterType.Collection);
				parameters.push(parameter);
			}
		}
		for (var i = 0; i < dashboard.Parameters.length; i++) {
			var dvParam = dashboard.Parameters[i];
			var parameter = GetParameter(this, dvParam.Name, dvParam.SerializedValue, dvParam.DataType, dvParam.Type == ParameterType.Collection);
			parameters.push(parameter);
		}
		return parameters;
	}
	...
	this.FakeQueryViewer = function (queryName) {
		...
		fakeQV.QueryInfo = "";
		fakeQV.AppSettings = [];
		fakeQV.AllowChangeAxesOrder = false;
	...
	this.InitializeQueryViewer = function (qViewer) {		//TODO: ESTO DEBERIA SER PARTE DEL QUERYVIEWER
		...
		qViewer.AllowChangeAxesOrder = false;
	...
	this.UpdateDropDownListFilter = function (control, name) {
		...
		if (filter != null) {
			var delimiter;
			if (filter.DataType == DataType.Character || filter.DataType == DataType.Date || filter.DataType == DataType.DateTime || filter.DataType == DataType.GUID)
				delimiter = '"';
			else
				delimiter = '';
			var jsonValue = "";
